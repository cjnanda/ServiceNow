...
encrypt: function(plaintext, password, nBits) {
    ...
    ciphertext = Packages.com.glide.util.StringUtil.base64Encode(ciphertext);
    return ciphertext;
},
decrypt: function(ciphertext, password, nBits) {
    ...
    var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
    if (!(nBits==128 || nBits==192 || nBits==256)) return ''; // standard allows 128/192/256 bit keys
    ciphertext = Packages.com.glide.util.StringUtil.base64Decode(ciphertext);
    var ciLen = ciphertext.length();
    //password = String(password).utf8Encode();
    ...
    var len = ciLen-8;
    var val = len/blockSize;
    var nBlocks = Math.ceil(val);
